...
*В этом репозитории уже реализована простейшая PWA‑поддержка: манифест и service worker.*
...
_API может быть запущено локально (см. раздел Backend ниже) или уже работает по адресу `https://kitek.ktkv.dev/marketplace/...`_
...
1. **Регистрация:**
```bash
# если вы запускаете бэкэнд локально, используйте адрес по умолчанию
# (порт 3001). Можно заменить на свой URL через переменную окружения
...
1. **Backend:**
   ...
   cd backend
   npm install
   npm run start    # запускает сервер на http://localhost:3001
   ```

2. **Frontend:**
   ```bash
   cd frontend
   npm install      # теперь без axios/jwt-decode/nanoid — они не используются
   # при необходимости создайте .env с VITE_API_BASE_URL
   npm run dev      # запускает приложение на http://localhost:5173 (или другом порту)
   ```

3. Откройте браузер на локальном адресе фронтенда и проверьте работу.
