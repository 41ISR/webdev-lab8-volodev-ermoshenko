...
> **Важно:** фронтенд по умолчанию обращается к адресу `http://localhost:3001/api`. Для запуска с другим сервером используйте
> переменную окружения `VITE_API_BASE_URL` (например, в файле `.env`).
...
> **Удалены / помечены как устаревшие:**
>
> * `frontend/src/store/UserStore.jsx` – раньше для хранения сессии, но теперь
>   используется `AuthContext`.
> * HTML‑шаблоны в `html-templates/` содержат только комментарии, они больше не
>   задействуются в приложении и могут быть удалены.
> * Зависимости `axios`, `jwt-decode`, `nanoid` удалены из `package.json`.
>   Эти библиотеки не используются в коде.
> * Файл `package.json` в корне (содержащий только `nodemod`) фактически нигде
>   не применяется и может быть удалён, если не нужен.
